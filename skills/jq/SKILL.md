...
description: >
  ...
  strings, dates, modules, invocation flags, and exit codes. Triggers on all
  jq-related topics: filters, programs, queries, the manual, builtins, regex,
  modules, reduce/foreach, select/walk/recurse/map, CLI flags (--arg, --argjson,
  --null-input, --raw-output, --compact-output, --stream, --exit-status),
  format strings (@base64, @csv, @sh, @uri), tojson/fromjson, date functions,
  $ENV/env, path functions (getpath, setpath, del, paths), sort/group_by,
  sub/gsub/test/match, with_entries/map_values, modulemeta/import, input/inputs,
  operators (//, destructuring), control flow (label, break, try, catch),
  and SQL-style builtins (INDEX, JOIN).
user-invocable: true
---
